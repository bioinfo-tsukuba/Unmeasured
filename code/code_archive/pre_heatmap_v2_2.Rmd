...
```{r}
anno_df <- df2 %>% filter(Genome == "mm10" & Antigen_class == "TFs and others")
dim(anno_df)
...
head(df7)
dim(df7)
saveRDS(df7, "/Users/saeko/Documents/MOCCS/important_chipseq_prediction/mm10_TF_CT_sample_num_new.rds")
...
```{r}
df7 <- readRDS("/Users/saeko/Documents/MOCCS/important_chipseq_prediction/mm10_TF_CT_sample_num_new.rds")
df7 <- t(df7)
dim(df7)
# rowごとにsample数集計
...
head(df7)
dim(df7)
saveRDS(df7, "/Users/saeko/Documents/MOCCS/important_chipseq_prediction/mm10_TF_CTC_sample_num_new.rds")
...
```
```{r}
df7 <- readRDS("/Users/saeko/Documents/MOCCS/important_chipseq_prediction/mm10_TF_CTC_sample_num_new.rds")
df7 <- t(df7)
dim(df7)
# rowごとにsample数集計
row_sum <- apply(df7, 1, sum)
...
Heatmap(df7, col = col_fun, cluster_rows = TRUE, cluster_columns = TRUE,  top_annotation = HeatmapAnnotation(sample_sum = anno_barplot(col_sum)), right_annotation =rowAnnotation(sample = anno_barplot(row_sum)),row_names_gp = gpar(fontsize = 4), column_names_gp = gpar(fontsize = 0))
```
# Cell type class(23) × TF Family(56) 
```{r}
# TF family のアノテーションをふる left_annotation = rowAnnotation(foo2 = 18:1, bar2 = anno_barplot(runif(18))) とか？
TF_fam <- read_tsv("/Users/saeko/Documents/MOCCS/important_chipseq_prediction/data/Mus_musculus_2023_01_12_12_51_am/TF_Information_mm10.txt")
head(TF_fam)
...
saveRDS(anno_df_fam, "/Users/saeko/Documents/MOCCS/important_chipseq_prediction/mm10_annotation_family_new.rds")
```
...
anno_df_fam <- readRDS("/Users/saeko/Documents/MOCCS/important_chipseq_prediction/mm10_annotation_family_new.rds")
...
head(df11)
dim(df11)
saveRDS(df11, "/Users/saeko/Documents/MOCCS/important_chipseq_prediction/mm10_TF_family_CTC_sample_num_new.rds")
...
# Histone 
```{r}
anno_df <- df2 %>% filter(Genome == "mm10" & Antigen_class == "Histone")
dim(anno_df)
length(unique(anno_df$ID))
```
...
head(df7)
dim(df7)
saveRDS(df7, "/Users/saeko/Documents/MOCCS/important_chipseq_prediction/Histone_mm10_TF_CT_sample_num_new.rds")
...
# Cell type class(24) × Antigen(118)
```{r}
df4 <- anno_df %>% group_by(Cell_type_class, Antigen) %>% summarize(num = n())
df4
...
head(df7)
dim(df7)
saveRDS(df7, "/Users/saeko/Documents/MOCCS/important_chipseq_prediction/Histone_mm10_TF_CTC_sample_num_new.rds")
...
```{r}
df7 <- readRDS("/Users/saeko/Documents/MOCCS/important_chipseq_prediction/Histone_mm10_TF_CTC_sample_num_new.rds")
df7 <- t(df7)
dim(df7)
# rowごとにsample数集計
row_sum <- apply(df7, 1, sum)
