# Knock TFとChIP-AtlasのCell typeのアノテーションを作り直してやり直した
rm(list=ls())
...
taiou <- read_tsv("/Users/saeko/Documents/MOCCS/important_chipseq_prediction/data/KnockTF_v2/KnockTF_tissue_ChIPAtlas_Ctc_taiou_v2.tsv")
colnames(taiou) <- c("Biosample_name", "Tissue", "Cell_type_class")
taiou2 <- taiou %>% select(Biosample_name, Cell_type_class)

stat_hu4 <- stat_hu4 %>% select(-Cell_type_class)
dim(stat_hu4)

stat_hu5 <- stat_hu4 %>% left_join(taiou2, by = "Biosample_name")
dim(stat_hu5)
length(unique(stat_hu5$Profile_ID))
length(unique(stat_hu5$TF.x))
length(unique(stat_hu5$Biosample_name))
length(unique(stat_hu5$Cell_type_class))
View(stat_hu5[1:10,])

colnames(stat_hu5)
stat_hu5 <- stat_hu5 %>% select(Target_Gene, TF.x, corrected_p,Cell_type_class) %>% distinct()
dim(stat_hu5)
...
write_tsv(stat_hu_DEG, "/Users/saeko/Documents/MOCCS/important_chipseq_prediction/data/KnockTF_v2/DEG_matrix_md/DEG_all_human_threshold_p_v2.tsv")
####################################################################################################################################################

stat_hu_DEG <- read_tsv("/Users/saeko/Documents/MOCCS/important_chipseq_prediction/data/KnockTF_v2/DEG_matrix_md/DEG_all_human_threshold_p_v2.tsv")
...
# count, 円グラフ -----
dim(df_join5)
tmp <- df_join5 %>% filter(label_chip_measure == "Measured") %>% select(TF, Cell_type_class, label_chip_measure) %>% distinct()
tmp2 <- df_join5 %>% filter(label_chip_measure == "Unmeasured") %>% select(TF, Cell_type_class, label_chip_measure) %>% distinct()
dim(tmp)
dim(tmp2)

categories <- c("Unmeasured", "Measured")
counts <- c(dim(tmp2)[1], dim(tmp)[1])
color2 <- c("blue", "red3")

old_par <- par(lwd = 3, cex = 1.5)
...
# TF markerの分割表---
df_measured <- df_join4 %>% filter(label_chip_measure == "Measured") %>% select(-Gene_type) %>% distinct()
View(df_measured)
dim(df_measured)
df_unmeasured <- df_join4 %>% filter(label_chip_measure == "Unmeasured") %>% select(-Gene_type) %>% distinct()
View(df_unmeasured)
dim(df_unmeasured)

df_join4 %>% ggplot(aes(x = label_chip_measure))
label_two <- df_join4$label_chip_measure
label_four <- df_join4 %>% unite("label", c(label_chip_measure, label_marker), sep = "_") %>% .$label
tmp <- df_join4 %>% mutate(bar = "un_measure") %>% mutate(label = label_two)
tmp2 <- df_join4 %>% mutate(bar = "category") %>% mutate(label = label_four)
tmp3 <- rbind(tmp, tmp2)

tmp3 %>% ggplot(aes(x = bar, fill = label)) +
  #geom_bar(stat = "identity", position = "fill") 
  geom_bar() +
  scale_fill_manual(values = c("red3",  "red3","pink3", "blue", "blue", "skyblue"))+
  xlab("")


tmp4 <- tmp3 %>% filter(bar == "category")%>% group_by(label) %>% summarise(n = n())

tmp4 %>% separate(label, c("label_measured", "label_marker"), sep = "_") %>% 
  ggplot(aes(x = label_measured, y = n, fill = label_marker))+
