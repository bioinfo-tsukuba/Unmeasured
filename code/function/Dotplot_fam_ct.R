Dotplot_fam_ct<- function(){
  ...
  # Cell type classごとに最大のサンプル数を持つCell typeを抽出
  max_sample_per_class <- df %>% group_by(Cell_type, Cell_type_class) %>% summarise(n_sample = n()) %>%
    group_by(Cell_type_class) %>%
    slice_max(n_sample)
  tgt_ct_list <- max_sample_per_class$Cell_type
  
  df3 <- df2 %>% left_join(tgt_df_fam, by = "Antigen") %>% 
    filter(Cell_type %in% tgt_ct_list) %>%
    group_by(Family, Cell_type) %>% 
  ...
  tmp2 <- tmp %>% group_by(Cell_type) %>% summarise(n_sum_sample_ct = sum(n_sample))
  tmp3 <- tmp %>% left_join(tmp2, by = "Cell_type")
  
  tmp4 <- tmp %>% group_by(Family) %>% summarise(n_sum_sample_fam = sum(n_sample))
  tmp5 <- tmp3 %>% left_join(tmp4, by = "Family")
  
  
  p <- tmp5 %>% 
    ggplot(aes(x = reorder(Family, -n_sum_sample_fam), y= reorder(Cell_type, n_sum_sample_ct), size = percentage, label = n_sample)) + 
    ...
    xlab("TF family") +
    ylab("Cell type") +
    ggtitle("Number of ChIP-seq samples") +
    theme(
      aspect.ratio = 0.7,
  ...
  p2 <- p + 
    ...
    geom_ysidecol(aes(x = n_sample, y = Cell_type), data = tmp, stat = "identity", width = 0.5, fill = "blue4") +
    theme(
