Dotplot_fam_ctc_unmeasured <- function(calc_opt, tgt_threshold){
  ...
  if(calc_opt == TRUE){
    ...
    # FPKM細胞ごとに上位25%であるTF-CTCの組み合わせをcount
    ...
    rownames(mt_totalTF) <- rownames(mt_Chip6)
    colnames(mt_totalTF) <- colnames(mt_Chip6)
    saveRDS(mt_totalTF, paste0("/Users/saeko/Unmeasured/data/mt_total_expressed_TF_",tgt_threshold,".rds"))
  }
  
  mt_totalTF <- readRDS(paste0("/Users/saeko/Unmeasured/data/mt_total_expressed_TF_", tgt_threshold,".rds"))
  ...
  mt_Chip6_tib <- mt_Chip6 %>% as_tibble() %>% mutate(Antigen = rownames_TF_chip) %>%
    left_join(df3_join, by = "Antigen") %>%
  ...
  tib_plot <-  mt_totalTF_tib %>% left_join(mt_Chip6_tib, by = c("Family", "Cell_type_class")) %>%
