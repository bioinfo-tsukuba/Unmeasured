UniqueTF_RRratio_scatter <- function(tgt_region){
  ...
  for (tgt_threshold in 1:10) {
    ...
    p1 <- df_binded1 %>% ggplot(aes(x = log(uniq_TF_num, base = 10), y = RR_cover_ratio, color = Cell_type)) +
      geom_point() +
      geom_line()+
    ...
    p2 <- df_binded1 %>% ggplot(aes(x = log(uniq_TF_num, base = 10), y = RR_cover_ratio, color = Cell_type)) +
      geom_point() +
      geom_line()+
      scale_color_manual(values = color_list) +
      ggtitle(paste0("Threshold = ", tgt_threshold)) +
