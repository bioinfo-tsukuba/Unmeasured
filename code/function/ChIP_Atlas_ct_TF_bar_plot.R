ChIP_Atlas_ct_TF_bar_plot <- function(calc_opt){
  ...
  if(calc_opt == TRUE){
    ...
    df2 <- df %>% filter(Antigen != "Epitope tags" & Antigen != "GFP" & Antigen != "Biotin") %>% 
      group_by(Cell_type, Antigen) %>% summarize(num = n())
    df3 <- df2 %>% pivot_wider(names_from = Cell_type, values_from = num)
    row_lab <- df3$Antigen
    df4 <- df3 %>% select(-Antigen)
    df5 <- df4 %>% as.matrix()
    rownames(df5) <- row_lab
    df5[which(is.na(df5))] <- 0
    saveRDS(df5, "/Users/saeko/Unmeasured/data/TF_CT_sample_num_new_20231004.rds")
    
    # ChIP-Atlas + Refseq df ----
    mt_Chip <- readRDS("/Users/saeko/Unmeasured/data/TF_CT_sample_num_new_20231004.rds")
    ...
    ######################ら
    ######################
    # Ct, tissueを2つの行列で揃える (RefEXとの対応をどうするかが問題)-----
    colnames(mt_Chip2)
    colnames(mt_RNA2)
